...
import { BaseExpansionEngine, Expansion, ExpansionEngine, RecipeExpansionEngine, RecursiveVariableExpansionException, ValueExpansionEngine, expandRecipe, expandValue } from "@src/cbuild-backend/expansion.js";
import { AssignmentIR, AssignmentType, ConditionalIR, ConditionKind, DefineIR, FunctionIR, HookIR, IncludeIR, NormalRuleIR, RecipeIR, StaticPatternRuleIR, TargetRuleIR, ValueIR, VpathIR, textPart, varRefPart } from "@src/compiler/ir.js";
...
describe("command recipes", () => {
  test.each(["", "echo hello", "  @-echo 'a b' | cat > out && echo $$HOME\n", "echo $(LITERAL)"])("preserves command text %j", command => {
    expect(expandRecipe(RecipeIR.command(literal(command)), env())).toBe(command);
  });
  ...
  test("interpolates raw, deferred and missing variables without quoting", () => {
    ...
    const command = new ValueIR([varRefPart(literal("CC")), textPart(" "), varRefPart(literal("FLAGS")), textPart(" "), varRefPart(literal("MISSING")), textPart("a.c")]);
    expect(expandRecipe(RecipeIR.command(command), context)).toBe("clang -O2 -g a.c");
    expect(context.requireVariable("FLAGS").isDeferred()).toBe(true);
    expect(context.hasVariable("MISSING")).toBe(false);
  });

  test.each(["helper", "recipe engine", "facade"])("%s starts fresh lookup state for every recipe", entry => {
    const context = env();
    ...
    const run = () => entry === "helper" ? expandRecipe(recipe, context) : entry === "recipe engine" ? engine.exec(recipe) : facade.expand(recipe);
  ...
  test("recipe expansion remains independent of facade value lookup state", () => {
    ...
    expect(() => engine.expand(reference("A"))).toThrow(RecursiveVariableExpansionException);
  });

  test("repeated deferred references within one recipe currently throw", () => {
    const context = env();
    context.setDeferredVariable("A", literal("ok"));
    const command = new ValueIR([...reference("A").parts, ...reference("A").parts]);
    expect(() => expandRecipe(RecipeIR.command(command), context)).toThrow("Recursive variable expansion: A");
  });

  test.each([
    ["comment", RecipeIR.comment("ignored?")],
    ["empty line", RecipeIR.emptyLine()],
    ["conditional", RecipeIR.conditional(new ConditionalIR(ConditionKind.IFDEF, { left: literal("A") }))],
...
describe("public entry points", () => {
  ...
  test("recipe sync and async execution agree", async () => {
    ...
    await expect(recipe.execAsync(new ExpansionEngine(context))).resolves.toBe("echo hello");
    await expect(recipe.execAsync(new RecipeExpansionEngine(context))).resolves.toBe("echo hello");
  });

  test("async entry points currently throw expansion errors synchronously", () => {
    const context = env();
    context.setDeferredVariable("A", reference("A"));
    expect(() => reference("A").execAsync(new ValueExpansionEngine(context))).toThrow(RecursiveVariableExpansionException);
    expect(() => reference("A").execAsync(new ExpansionEngine(context))).toThrow(RecursiveVariableExpansionException);
    expect(() => RecipeIR.command(reference("A")).execAsync(new RecipeExpansionEngine(context))).toThrow(RecursiveVariableExpansionException);
  });

  test.each([
    ["assignment", new AssignmentIR(AssignmentType.SIMPLE, literal("A"), literal("B"))],
    ["function", new FunctionIR("shell")],
    ["conditional", new ConditionalIR(ConditionKind.IFDEF, {})],
  ...
  ] as const)("rejects unsupported %s IR without changing the environment", (_name, input) => {
    const context = env();
    const engine = new ExpansionEngine(context);
    expect(() => input.exec(engine)).toThrow("Unsupported IR node for expansion");
    expect(() => input.execAsync(engine)).toThrow("Unsupported IR node for expansion");
  ...
  test("Expansion.expand discards results for a single node or an array", () => {
    ...
    expect(expansion.expand([literal("one"), RecipeIR.command(literal("two"))], context)).toBeUndefined();
  });
  ...
  test("Expansion.expand shares value lookup state within a batch but not across calls", () => {
    ...
    context.setDeferredVariable("A", literal("ok"));
    const expansion = new Expansion();
    expect(() => expansion.expand([reference("A"), reference("A")], context)).toThrow("Recursive variable expansion: A");
    expect(() => expansion.expand(reference("A"), context)).not.toThrow();
    expect(() => expansion.expand(reference("A"), context)).not.toThrow();
    expect(() => expansion.expand([RecipeIR.command(reference("A")), RecipeIR.command(reference("A"))], context)).not.toThrow();
  });
});
